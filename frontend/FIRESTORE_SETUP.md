...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // Cards - QUAN TRỌNG: Cards ownership qua parent collection
    match /cards/{cardId} {
      // Get the parent collection to check ownership
      function getCollection() {
        return get(/databases/$(database)/documents/collections/$(resource.data.collection_id));
      }
      
      // Allow read if user owns the parent collection
      allow read: if isSignedIn() && getCollection().data.user_id == request.auth.uid;
      
      // Allow create if parent collection belongs to user
      allow create: if isSignedIn() && 
        get(/databases/$(database)/documents/collections/$(request.resource.data.collection_id)).data.user_id == request.auth.uid;
      
      // Allow update/delete if parent collection belongs to user
      allow update, delete: if isSignedIn() && getCollection().data.user_id == request.auth.uid;
    }
